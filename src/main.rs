...
use game::{Cords, Life, N_GRID};
use piston_window::{
    AdvancedWindow, Button, EventLoop, Key, MouseButton, MouseCursorEvent, MouseRelativeEvent,
    PistonWindow, PressEvent, WindowSettings, Rectangle,
};

mod game;

fn main() {
    let mut life = Life::new(vec![Cords::new(5, 1), Cords::new(5, 2), Cords::new(5, 3)]);
            ...
            match btn {
                ...
                Button::Mouse(click) => {
                    if click == MouseButton::Left {
                        let x = (cursor[0] / 10.0) as usize;
                        let y = (cursor[1] / 10.0) as usize;
                        life.toggle(x, y);
                    }
                }
                _ => {}
